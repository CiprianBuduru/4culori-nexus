...
export function EmailDraftPanel({
  ...
  const renderPreviewHtml = () => {
    ...
    const productBlocks = safeProducts
      .map((p) => {
        const specs = buildClientSpecs(p.configSnapshot);
        const specItems: string[] = [];
        specItems.push(`<li>Tiraj: ${p.quantity ?? 0} buc</li>`);
        specs.forEach((s) => specItems.push(`<li>${s.label}: ${s.value}</li>`));
        if (!p.configSnapshot && p.details) {
          specItems.push(`<li>${p.details}</li>`);
        }
